...
per_sec = 0
per_sec_time = time.time()
ans = 0

while True:
  iddd = 69
  per_sec += 1
  if time.time() - per_sec_time >= 1:
    ans = per_sec
    per_sec = 0
    per_sec_time = time.time()
  ...
  if command == 'mm_process':
    ...
    ti_me.execute(f"INSERT INTO ti_me VALUES({iddd}, {ans}, -1)")
    conn9.commit()
    ...
    if ENABLE_OUTPUT: print("Unknown command.....")
    continue
  print("Per second:", ans)
  conn.commit()
  conn1.commit()
  conn2.commit()
  conn3.commit()
