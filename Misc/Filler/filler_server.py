...
def box_graph(product, buy_sell, new):
  ...
  print("Adding to box", datetime.fromtimestamp(ti_me), '\n\n')
  b.execute(f"INSERT INTO box VALUES('{product}', {best_bid}, {best_ask}, {ti_me})")
...
def register(login, password):
    ...
    if len(u.fetchall()) != 0:
        return False
    u.execute(f"INSERT INTO users VALUES({ti_me}, '{login}', '{password}', 1000)")
    conn3.commit()
    return True
...
def time_skip():
  ...
  while True:
    ti_me += 3
    time.sleep(0.001)
    if ti_me > time.time() + 3600*24: break
