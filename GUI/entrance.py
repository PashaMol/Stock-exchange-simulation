...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        L.addWidget(QtWidgets.QLabel("Name"), 0,0)
        L.addWidget(self.uNameLine, 0,1)
       # L.addWidget(QtWidgets.QLabel("ID"), 1, 0)
        #L.addWidget(self.idLine, 1, 1)
        L.addWidget(QtWidgets.QLabel("IP"), 2, 0)
        L.addWidget(self.IPline, 2,1)
        L.addWidget(QtWidgets.QLabel("Password"), 3, 0)
        L.addWidget(self.uPasswordLine, 3, 1)
        self.IPline.setText(client.IP)
        L.addWidget(self.signInButton)
        L.addWidget(self.RegBut)
        self.infLabel = QtWidgets.QLabel("")
        L.addWidget(self.infLabel, 5, 1)
        #L.addWidget()
    ...
    def signingIn(self):
        ...
        if len(data.username)!=0 and len(data.userid)!=0 and len(data.password)!=0 and len( client.IP)!=0 and functions.is_number(data.userid):
            ...
            try:
                 ...
                 mainWindow.runGUI()
            except:print("Error while starting app")
        else:
