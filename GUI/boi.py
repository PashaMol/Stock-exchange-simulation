...
def add_asset(id, product, amount):
  a.execute(f"SELECT * FROM assets WHERE product = '{product}'")
  fet = a.fetchall()
  if len(fet) == 0:
    a.execute(f"INSERT INTO assets VALUES({id}, '{product}', '{amount}')")
  else:
    a.execute(f"UPDATE assets SET amount = {float(fet[0][2]) + float(amount)}")
  ...
  if buy:
    ...
    for i in c.fetchall():
      ...
      if i[5] > q:
        ...
        add_asset(uid, product, float(amount))
        add_asset(i[0], product, -1*float(amount))
        ...
        add_asset(uid, product, i[5])
        add_asset(i[0], product, -1*i[5])
        add_history(login, product, i[5], i[5]*i[6], "buy")
        calc_average(product, -1*i[6], 'sell')   #Do we ned that?
        box_graph(product, 'sell')
        list_counter += 1
    if q != 0 and limit:
      add_debt(login, reqid, q*price)
      substract(True, q*price, login)
      add_to_buffer(['add', reqid, from_u, b[2] ,b[3], product, str(q), price, uid])
      c.execute("INSERT INTO orders VALUES(" + str(reqid) + ", '" + str(from_u) + "', '" + b[2] + "', '" + b[3] + "', '" + product + "', " + str(q) + ", " + str(price) + ", " + str(uid) + ")")
    ...
    for i in c.fetchall():
      ...
      if i[5] > q:
        ...
        add_to_buffer(['update', reqid, i[5]-q])
        c.execute("UPDATE orders SET amount = '" + str(i[5] - q) + "' WHERE reqid =" + "\'" + str(i[0]) + "\'")
        add_asset(uid, product, -1*float(amount))
        add_asset(i[0], product, float(amount))
        ...
        add_to_buffer(['delete', reqid])
        c.execute("DELETE FROM orders WHERE reqid =" + "\'" + str(i[0]) + "\'")
        add_asset(uid, product, -1*i[5])
        add_asset(i[0], product, i[5])
    ...
    if q != 0 and limit:
      c.execute(f"INSERT INTO orders VALUES({reqid}, '{from_u}', '{b[2]}', '{b[3]}', '{product}', {q}, {price}, {uid})")
      add_to_buffer(['add', reqid, from_u, b[2] ,b[3], product, str(q), price, uid])
      calc_average(product, price, 'sell')    #Do wee need that?
      box_graph(product, 'sell')
  substract(buy, total, login)
  # print()
...
global u, conn3, a, conn4, d, conn5, h, conn6
...
while True:
  ...
  if command == 'get':
    ...
    if not find(login, password):
      ...
      client_socket.send(pickle.dumps([False]))
      continue
    if float(got[5])*float(got[6]) > get_balance(login):
      if ENABLE_OUTPUT: print("Not enough money.....")
      client_socket.send(pickle.dumps([False]))
      continue
    if got[3] == 'sell' and not does_have(got[1], got[4], got[5]):
